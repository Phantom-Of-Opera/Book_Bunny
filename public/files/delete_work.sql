CREATE OR REPLACE FUNCTION delete_reader_cleanup(book_id INT, reader_id INT)
RETURNS void AS
$$
BEGIN
  WITH deleted_reader_book AS (
    DELETE FROM books_readers
    WHERE id_book = book_id AND id_reader = reader_id
    RETURNING id_book
  ),
  books_to_delete AS (
    SELECT b.id_book
    ...
    WHERE br.id_book IS NULL
      AND b.id_book IN (SELECT id_book FROM deleted_reader_book)
  ),
